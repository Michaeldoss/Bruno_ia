...
Laser Fiber 20W/30W/50W | Area: 200x200mm

--- PLOTTER DE RECORTE ---

DG1351 | Largura midia: 1300mm | Corte: 1220mm
  Velocidade: ate 800mm/s | Forca: 500g-1000g
...
Antes de qualquer conversa sobre CNPJ, proposta ou fechamento, apresente a tinta correspondente ao equipamento discutido.

REGRAS DE NEGOCIACAO E CREDITO:
- Aprovacao de credito (boleto) EXIGE CNPJ.
- Simples Nacional, Lucro Presumido, Lucro Real, LTDA, SA: APROVADO para boleto normalmente.
- MEI: precisa de avaliacao pelo financeiro.
- Pessoa Fisica (CPF, sem CNPJ): apenas cartao de credito ou a vista.
...
# ---------------------------------------------------------------------------
# System prompt simplificado para Haiku (sem docs, sem base de conhecimento)
# Haiku só faz roteamento de saudações simples — não precisa do catálogo completo
# ---------------------------------------------------------------------------
...
TOM: direto, consultivo, sem emojis, máximo 3 linhas por mensagem, sempre termine com CTA.

REGRAS:
- NUNCA repita pergunta já respondida
- NUNCA diga "não sei" ou "estou à disposição"
- NUNCA use "mano", "cara" ou gírias de gênero
- Zero emojis

Na abertura: pergunte nome e cidade na mesma frase.
Quando o cliente mencionar produto, preço ou assunto técnico: responda brevemente e sinalize que tem mais detalhes disponíveis.

Produtos principais: Plotters eco solvente/sublimática (DG1801i R$58.900, DG1802i R$68.900), DTF Têxtil (3002 R$52.900, 6002 R$92.900), DTF UV, UV Plana, Laser.
...
async def process_message_with_assistant(thread_id: str, user_message: str) -> list:
    ...
    try:
            ...
            if len(cnpj_clean) == 14 and not e_pergunta:
                ...
                if "error" in cnpj_data:
                    ...
                    if regime == "MEI":
                        ...
                        instrucao_vendedor = "TRATATIVA MEI: financeiro faz analise personalizada."
                    elif not ativo:
                        ...
                        instrucao_vendedor = "ATENCAO: CNPJ inativo na Receita Federal."
                        ...
                        instrucao_vendedor = f"RISCO: score {score_s}/1000, negativos presentes."
                    elif negativos:
                        ...
                        instrucao_vendedor = f"ATENCAO: negativos presentes (score {score_s}/1000)."
                        ...
                        instrucao_vendedor = f"APROVADO para boleto. Regime: {regime}. Score: {score_s}/1000."
                    else:
                        parecer       = "VERIFICAR"
                        instrucao_bruno = (
                        ...
                        instrucao_vendedor = f"Regime: {regime} | Score: {score_s}/1000."
        ...
        # ── Busca histórico — LIMITADO A 20 MENSAGENS (reduzido de 40)
        # Isso reduz tokens de histórico em ~50% sem perder contexto relevante
        raw_history = (
            db.query(Conversation)
            .filter(Conversation.phone == phone)
            .order_by(Conversation.created_at.asc())
            .limit(20)  # ← ALTERADO: era 40, agora 20
            .all()
        )
        ...
        # ── Decide modelo antes de montar o system prompt ─────────────────
        model = choose_model(user_message)

        # ── Monta tabela de preços em tempo real (apenas para Sonnet) ──────
        # Haiku não precisa da tabela dinâmica — economiza 1 call ao Sheets
        ...
        # ── Monta system prompt baseado no modelo ─────────────────────────
        # HAIKU: prompt mínimo (~300 tokens) — só para saudações simples
        # SONNET: prompt completo com cache (~13.000 tokens, pago 1x por sessão)
        ...
        # ── Parte dinâmica (muda por chamada — NÃO deve entrar no cache) ──
        system_dinamico = ""
        if customer_data:
        ...
        # ── Monta system como lista com cache_control na parte estática ───
        # cache_control "ephemeral" = TTL 5 minutos na Anthropic
        # Economiza ~65% do custo de input em conversas com múltiplos turnos
        system_parts = [
            {
                "type": "text",
        ...
        try:
            response = await asyncio.wait_for(
                client.messages.create(
                    ...
                    max_tokens=1024,
                    temperature=0.4,
                    system=system_parts,   # ← lista com cache_control
                    messages=messages,
                ),
                timeout=25.0
            )
                        ...
                        if cd and "error" not in cd:
                            ...
                            if raw_serasa and len(raw_serasa) > 50:
                                ...
                                if _regime == "MEI":
                                    ...
                                    _orient  = f"Score {_score}/1000. Negativos detectados."
